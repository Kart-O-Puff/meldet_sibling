"""
MelDet Approach V3: Melodic Similarity Detection using N-gram Analysis and Edit Distance
...
def find_best_diagonal(matrix, is_similarity=True):
    """Find scores for all diagonal alignments in a matrix."""
    m, n = matrix.shape
    min_len = min(m, n)
    max_shift = abs(m - n) + 1
    scores = []
...
def calculate_similarity_score(matrix, forced_shift=None):
    """Calculate similarity score using average of all diagonals."""
    ...
    # Get scores for all diagonals
    scores = find_best_diagonal(matrix)
...
def plot_matrix_as_table(matrix, seq1, seq2, title, is_similarity=False, similarity_score=None, song1="", song2="", forced_diagonal=None):
    """Visualize similarity/distance matrix with detailed values and all diagonals."""
    ...
    for i in range(len(seq1)):
        for j in range(len(seq2)):
            ...
            # Highlight cells for all possible diagonals
            if forced_diagonal is not None:
                ...
                for shift in range(max_shift):
                    if m <= n and i < min_len and j == i + shift:
                        cell.set_edgecolor('red')
                        cell.set_linewidth(2)
                    elif m > n and j < min_len and i == j + shift:
                        cell.set_edgecolor('red')
                        cell.set_linewidth(2)
...
def plot_heatmap(matrix, seq1, seq2, title, is_similarity=False, similarity_score=None, song1="", song2="", forced_diagonal=None):
    """Plot similarity/distance matrix as a heatmap with all diagonals."""
    plt.figure(figsize=(10, 8))
    ...
    # Plot all diagonals or forced diagonal
    m, n = matrix.shape
    min_len = min(m, n)
    max_shift = abs(m - n) + 1
    ...
    if forced_diagonal is not None:
        if m <= n:
            plt.plot(range(forced_diagonal, forced_diagonal + min_len),
                    range(min_len),
                    'rx-', markersize=8, linewidth=2, label='Diagonal')
        else:
        ...
        for shift in range(max_shift):
            if m <= n:
                plt.plot(range(shift, shift + min_len),
                        range(min_len),
                        'rx-', markersize=4, linewidth=1, alpha=0.5, label=f'Diagonal {shift+1}' if shift == 0 else "")
            else:
                plt.plot(range(min_len),
                        range(shift, shift + min_len),
                        'rx-', markersize=4, linewidth=1, alpha=0.5, label=f'Diagonal {shift+1}' if shift == 0 else "")
...
def analyze_case(df, case_number, show_visualizations=False):
    ...
    # Calculate similarity scores using all diagonals
...
def analyze_all_cases(df, show_plots=False):
    ...
    for case in cases:
        ...
        # Calculate scores using all diagonals
        pitch_similarity = calculate_similarity_score(pitch_similarities)
        rhythm_similarity = calculate_similarity_score(rhythm_similarities)
...
def main():
    ...
    report_path = Path(__file__).parent / "similarity_report_meldet_v3.csv"
